...
# Run DockFlow
DockFlow --project test --protocol vina -r receptor.mol2 -l compounds.mol2 --radius 12 --center 20.259 -2.752 18.203 -sf vina <<EOF
n
EOF
}
...
# Run DockFlow
DockFlow --project test --protocol vina -r receptor.mol2 -l compounds.mol2 --radius 12 --center 20.259 -2.752 18.203 -sf vina --overwrite <<EOF
Y
n
EOF
}
...
DockFlow --project test --protocol plants -r receptor.mol2 -l compounds.mol2 --radius 12 --center 20.259 -2.752 18.203 <<EOF
n
EOF
}

run_plants_overwrite_no_continue(){
DockFlow --project test --protocol plants -r receptor.mol2 -l compounds.mol2 --radius 12 --center 20.259 -2.752 18.203 --overwrite <<EOF
y
n
EOF
}
...
FILES="test.chemflow/DockFlow/vina1/receptor/CHEMBL477992/VINA/output.pdbqt"
...
FILES="test.chemflow/DockFlow/vina1/receptor/DockFlow.csv test.chemflow/DockFlow/vina1/receptor/docked_ligands.mol2"
for FILE in ${FILES} ; do
    msg="The file ${FILE} has not been created.  "
    assertFileExits
done
...
if [ "${error}" != "true" ] ; then
    ...
    echo "[ TestFlow ] ${FAILED} over ${TOTAL} tests failed... Fix this !                       "
fi