...
"DockFlow")
    ...
    *)
	...
	echo "Docking parameters read from the input configuration file"
		#		check_config
	fi
    ;;
...
check_center(){
if [ "${DOCK_PROGRAM}" == "PLANTS" ] || [  "${DOCK_PROGRAM}" == "VINA" ]  || [  "${DOCK_PROGRAM}" ==  "QVINA" ]; then
 if [ -z "${POSTPROCESS}" ] && [ -z "${ARCHIVE}" ]; then
...
check_config(){
if [ "${DOCK_PROGRAM}" == "SMINA" ] ||  [ "${DOCK_PROGRAM}" == "PLANTS" ] ; then
  if [ -z "${conf_file}" ] ; then
