...
"None")
    ...
    "SMINA")
		...
		echo "echo [ Docking ] ${RECEPTOR_NAME} - ${LIGAND} ; smina.static --receptor ${RUNDIR}/receptor.pdbqt --ligand ${RUNDIR}/${LIGAND}/ligand.pdbqt --config ${WORKDIR}/${conf_file} --out ${RUNDIR}/${LIGAND}/SMINA/output.pdbqt --log ${RUNDIR}/${LIGAND}/SMINA/output.log " >> dock.xargs
            else
...
#                Diego E. B. Gomes
#
#        UPDATE: Mon Oct 11 16:03:06 CDT 2021
#
#===============================================================================
cp ${RECEPTOR_FILE} ${RUNDIR}/receptor.mol2
 case ${DOCK_PROGRAM} in

# MGLToolsPckgs/AutoDockTools/Utilities24/ must be in the PATH, so full paths are not necessary
#    "VINA" )
...
for LIGAND in ${LIGAND_LIST[@]} ; do
    ...
    if  [ ! -f  ${LIGAND}/ligand.pdbqt ] ; then
        prepare_ligand4.py \
        -l ${WORKDIR}/${PROJECT}.chemflow/DockFlow/${PROTOCOL}/input/${LIGAND}.mol2 \
        -o ${LIGAND}/ligand.pdbqt -U 'lps'
    fi
   ;;
    esac
done
