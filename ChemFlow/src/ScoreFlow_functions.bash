...
#        echo "tleap -f ../tleap_SALT.in &> tleap.job" > ScoreFlow.run.template
      
#	echo "tleap -f ../tleap.in &> tleap.job"  >  ScoreFlow.template
        echo "tleap_SALT.bash &> tleap.job" > ScoreFlow.template
  #     if [ -f ${RUNDIR}/${LIGAND}/ionized_solvated.rst7 ] && [ ${WATER} == 'yes' ] ; then

 #      WAT=$(cat ionized_solvated.prmtop | grep WAT | sed 's/ /\n/g' | grep WAT | wc -l)
...
#       echo "tleap -f ../tleap_salt.in &> tleap2.job" > ScoreFlow.template
#       fi
...
#            echo -e "cd ${RUNDIR}/${LIGAND}" > ScoreFlow.run
            ...
            if [ ! -f ${RUNDIR}/${LIGAND}/ionized_solvated.rst7 ] && [ ${WATER} == 'yes' ] ; then
#                echo "tleap -f ../tleap_SALT.in &> tleap.job" >> ScoreFlow.run
#               echo "tleap -f ../tleap.in &> tleap.job" >> ScoreFlow.run 
		eval ""$(echo "bash ../tleap_SALT.bash")" &> tleap.job" >> ScoreFlow.run               

echo "MODIFICATO SU"
	    fi
#	      if [ -f ${RUNDIR}/${LIGAND}/ionized_solvated.rst7 ] && [ ${WATER} == 'yes' ] ; then
# 
#	       echo "tleap -f ../tleap_salt.in &> tleap2.job" >> ScoreFlow.run
#       fi
...
#eval echo \""${file}"\" >> ${RUNDIR}/tleap_SALT.bash
#sed "1i\\" ${file} >> ${RUNDIR}/tleap_SALT.bash
