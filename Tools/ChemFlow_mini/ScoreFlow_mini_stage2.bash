...
#
# ScoreFlow stage 2 - Running the MD in GBSA for 1ns.
#

ScoreFlow_ligand="DockFlow_TOP"
ScoreFlow_input="ScoreFlow/input"
ScoreFlow_output="ScoreFlow/MMGBSA_implicit"
...
parametrized=""
for i in ${list[@]} ; do
  if [ -f ${ScoreFlow_parameters}/${i}/lig_bcc.mol2 ] ; then
...
for i in ${list[@]} ; do
  ...
  cp ${rundir}/${ScoreFlow_parameters}/${i}/{lig_bcc.mol2,lig.frcmod} ${rundir}/${ScoreFlow_output}/${i}/  
done
if [ "${DEBUG}" == 1 ] ; then echo -ne "\n[COPY Ligand] DONE \n" ; fi
...
tleap -f tleap_gbsa.in &> tleap.job
}
