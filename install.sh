...
_check(){
  ...
  if [ -z "$mgltools_folder" ]; then
    ...
    if [ -z "$mgltools_folder" ]; then
      echo "[ WARNING ] MGLTools could not be found. Please install it and run this script if you plan on using Vina, Qvina or Smina."
      let warning_count+=1
    else
      if [ -z "$1" ]; then
        # Add to .bashrc
  ...
  if [ -z "$(command -v qvina)" ];            then not_on_path WARNING Qvina; fi
  if [ -z "$(command -v smina)" ];            then not_on_path WARNING Smina; fi
